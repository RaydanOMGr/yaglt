...
## `libEGL.so` drop-in wrapper (transparent interception) — implemented

YAGLT ships a **`libEGL.so` shim** (see `src/egl_shim/`) that mirrors LTW's
libEGL wrapper trick: it is both the EGL API surface the application links against
and the dispatcher to the real driver. Build it with the `YAGLT_EGL_SHIM=ON` option
(requires `YAGLT_BACKEND_GLES=ON`); the target `yaglt_egl_shim` produces a shared
library named `libEGL.so`.

Mechanism:
- At load time a constructor `dlopen`s the **real** system `libEGL` (path from the
  `YAGLT_HOST_EGL` environment variable, default `libEGL.so.1`) and resolves every
  forwarded EGL entry point. The shim's SONAME is `libEGL.so` (not `libEGL.so.1`), so
  the GLES backend's own `dlopen("libEGL.so.1")` finds the real driver, not the shim —
  no recursion.
- Every EGL 1.x entry point (`eglGetDisplay`, `eglChooseConfig`, `eglCreateContext`,
  `eglMakeCurrent`, `eglSwapBuffers`, `eglQueryString`, images/syncs, platform
  surfaces, device queries, …) is forwarded verbatim to the host driver.
- The full desktop-GL 4.6 surface is exported as C `gl*` symbols (generated by
  `tools/gen_gl_exports.py` from `glcompat/frontend/gl_api.hpp`, so it never drifts)
  that route into YAGLT's frontend. `eglGetProcAddress` returns YAGLT's `gl*` for
  `gl…` names and forwards everything else (EGL extensions, GLES-only entry points)
  to the host; `glXGetProcAddress`/`glXGetProcAddressARB` are aliased to it for
  desktop-GL apps.

Context lifecycle (owning the `Context`):
- `eglCreateContext` forwards to the host to obtain the real EGL context, then
  creates a `glcompat::Context` backed by the GLES backend and records it per real
  EGL context.
- `eglMakeCurrent` forwards to the host (so the application's surface is current),
  then marks the YAGLT `Context` current. On first activation the GLES backend
  **adopts** the application's real EGL display/context (`GLESBackend::setAdopt`) so
  YAGLT issues native GL on that exact context and therefore renders into the
  application's own surface. `eglDestroyContext` tears the mapping down.

Goal: seamless integration — an unmodified EGL/desktop-GL application is transparently
translated/extended by YAGLT without code changes. Absent driver → honest failure
(EGL calls return safe defaults; GL calls report no current context).

Deployment example:
```
YAGLT_HOST_EGL=/usr/lib/libEGL.so.1 \
LD_LIBRARY_PATH=/path/to/shim:/path/to/mesa \
EGL_PLATFORM=surfaceless ./my_app
```
`tests/integration/egl_shim_smoke.cpp` drives a real Mesa EGL stack through the shim
and asserts that `glGetString(GL_VERSION)` reports YAGLT.
