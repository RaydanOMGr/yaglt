...
| Shader/Program queries | Implemented | `glGetShaderiv` covers SHADER_TYPE, COMPILE_STATUS, DELETE_STATUS, SHADER_SOURCE_LENGTH, INFO_LOG_LENGTH; unknown pname → `GL_INVALID_ENUM` (SPEC §7.3). `glGetProgramiv` per above (SPEC §7.14). Verified by `shader_program_query_test` |
