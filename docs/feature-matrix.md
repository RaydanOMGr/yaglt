...
| State tracking | Implemented | `GLStateTracker` + `GLStateSink`; Mock & GLES backends flush via `Context::flushState()`/`glFlushState()`. Viewport (`glViewport`) and scissor box (`glScissor`) now tracked and pushed only on change; the scissor *test* is `GL_SCISSOR_TEST` capability (enable/disable), distinct from the box. Verified by `viewport_scissor_test` |
