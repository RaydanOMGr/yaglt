...
| Clear | Implemented | `glClearColor`/`glClearDepth`/`glClearDepthf` record the per-context clear values in `GLStateTracker` (pushed only on change, SPEC §10); `glClear` validates the mask (bits outside color/depth/stencil → `GL_INVALID_VALUE`) then flushes tracked state and issues the native clear via `IGraphicsBackend::clear`. Mock records the push; GLES drives `glClearColor`/`glClearDepthf`/`glClear` through `GLESLib`. Verified by `clear_test` |
